...
export default function ChatInterface() {
  ...
  const { messages, input, handleInputChange, handleSubmit, isLoading, stop, append, setInput } = useChat({
    api: '/api/chat',
  ...
  const statusLabel =
    agentPhase === 'thinking' ? 'Processing...' :
    agentPhase === 'graph'    ? 'Ready'         : 'Standby';
    ...
    <>
      ...
      <div className="app">
        ...
        <div className="topbar">
          <div className="topbar-logo">mastik<span>Q</span></div>
          ...
          <div className="right-panel">
                  ...
                  return (
                    <div key={msg.id} className="msg-assistant-group">
                      {msg.parts?.map((part, i) => {
                        if (part.type === 'text') {
                          return part.text ? <AiMessage key={i} content={part.text} /> : null;
                        }
                        if (part.type === 'tool-invocation') {
                          ...
                          return (
                            <ToolCard
                              ...
                              result={isDone ? (inv as { result: unknown }).result : undefined}
                            />
                          );
                        }
                        return null;
            ...
            <div className="chat-input-area">
              <form onSubmit={onSend}>
                <div className={`input-row${isRecording ? ' is-recording' : ''}`}>
                  {/* Recording badge */}
                  {isRecording && (
                  ...
                  {isLoading ? (
                    <button
                      ...
                      onClick={() => { stop(); setAgentPhase('idle'); }}
